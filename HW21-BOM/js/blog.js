...
function addListenersForSearchButton() {
  const searchField = document.getElementById('searchField');
  const blogSearchButtons = document.querySelectorAll('.blog__search-buttons-item');
  const searchByAuthorButton = blogSearchButtons[0];
  const searchByTitleButton = blogSearchButtons[1];

  searchByAuthorButton.addEventListener('click', () => {
    localStorage.setItem('name', searchField.value);
    renderFilteredArticleFromJSON(searchField.value);
  });
  searchByTitleButton.addEventListener('click', () => {
    localStorage.setItem('title', searchField.value);
    renderFilteredArticleFromJSON('title', searchField.value);
  });
}
function getFilteredArticle(listOfPosts) {
  const filteredPosts = [];
  for (let i = 0; i < listOfPosts.length; i++) {
    ((listOfPosts[i].name === localStorage.getItem('name'))) ? filteredPosts.push(listOfPosts[i]) : filteredPosts;
  }

  return filteredPosts;
}
async function renderFilteredArticleFromJSON() {
  try {
    const response = await fetch('../../server/config/articles.json');
    let article;
    if (response.ok) {
    ...
    article = getFilteredArticle(article);
    makeAppendChild(main,
      headerDraw(data.find((b) => b.id === 'header')),
      createPostInstances(article),
      createWholePostInstances(article),
    ...
    readMoreButtonRedirect(article);
    addListenersForSearchButton(article);
  } catch (err) {
    console.warn(err);
  }
}
...
function getBlogSearchTemplate(data, section) {
  return `<input class="${section}-field" id="searchField" type="search" placeholder="Search by author" value="Neil Richards" />
...
function getBlogSearchButtonTemplate(section) {
  return `<button class="${section}-item">By author</button>
          <button class="${section}-item">By title</button>`;
}
...
const headerDraw = function (headerData) {
  ...
  const blogSearchButton = drawElement('div', `${blog.className}__search-buttons`);
  blogSearchButton.innerHTML = getBlogSearchButtonTemplate(blogSearchButton.className);
  ...
  makeAppendChild(blog, wrapBlogHeader, blogSearchBlock, blogSearchButton);
