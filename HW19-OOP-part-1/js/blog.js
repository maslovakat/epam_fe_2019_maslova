...
function getQuantityOfStars(post) {
  const starList = [];
  for (let i = 1; i < 6; i++) {
    starList.push(getQuantityOfStarsItemTemplate(post, i));
  }
  return starList.join(' ');
}
function getQuantityOfStarsItemTemplate(post, number) {
...
function getRightTypeOfMediaTemplate(post) {
...
function getLatestPostsBlockTemplate(post) {
  return `<div class="post">
            <div class="post__picture">${getRightTypeOfMediaTemplate(post)}</div>
...
function getWholePostsBlockTemplate(post) {
...
const postDraw = function (postData) {
  ...
  postsBlock.innerHTML = postData.posts.map((i) => getLatestPostsBlockTemplate(i)).join('');
...
const wholePostDraw = function (wholePostData) {
  ...
  wholePostsBlock.innerHTML = wholePostData.wholePosts.map((i) => getWholePostsBlockTemplate(i)).join('');
